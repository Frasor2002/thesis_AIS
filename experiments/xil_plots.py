...
def save_metric_table_pdf(results_dict: Union[Dict, List[Dict]], value_key: str, measure: str, dataset_name: str, scenario_name: str):
  ...
  data_dict = {}
  active_strategies = [] # Track which strategies actually have data
  ...
  for strat in STRATEGIES:
    ...
    active_strategies.append(strat) # Only add strategies that exist
  ...
  # Safety check: If NO strategies had data, skip creating the table
  if not active_strategies:
    print(f"No data available to generate table for {dataset_name} {scenario_name}")
    return

  sorted_queries = sorted(data_dict.keys())
  
  # Build columns and rows dynamically based ONLY on active strategies
  columns = ["Explained Samples"] + active_strategies
  cell_text = [[q] + [data_dict[q].get(strat, "-") for strat in active_strategies] for q in sorted_queries]
